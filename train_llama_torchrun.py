"""
torchrun용 Llama 70B 멀티노드 분산 훈련 스크립트
"""
import os
import sys
import math
...
from distributed_utils import (
    cleanup_distributed, get_rank, get_world_size,
...
def parse_args():
    parser = argparse.ArgumentParser(description='Llama 70B torchrun 멀티노드 훈련')
...
def setup_distributed_torchrun():
    """torchrun 환경에서 분산 훈련 설정"""
    # torchrun이 자동으로 설정하는 환경 변수들 사용
    ...
    # 분산 프로세스 그룹 초기화 (torchrun이 자동으로 환경 변수 설정)
    dist.init_process_group(backend='nccl')
    
    logger.info(f"프로세스 {rank}/{world_size} 초기화 완료 (local_rank: {local_rank}, GPU: {torch.cuda.current_device()})")
...
def main():
    ...
    # torchrun 분산 훈련 설정
    local_rank, rank, world_size = setup_distributed_torchrun()
    ...
    for epoch in range(start_epoch, args.max_epochs):
        ...
        save_checkpoint(
            ...
            avg_loss, args.checkpoint_dir, rank
        )
